...
try {
    $response = $client->get('https://api.github.com/repos/runk/npm-proxy-cache/git/refs/tags');
...
$folder = getcwd() . DIRECTORY_SEPARATOR . 'npm-proxy-cache';
if (!$fs->exists($folder)) {
    $fs->mkdir($folder);
}
...
foreach ($versions as $version) {
    $content = <<<EOF
NPM_PROXY_CACHE_VERSION="$version"
